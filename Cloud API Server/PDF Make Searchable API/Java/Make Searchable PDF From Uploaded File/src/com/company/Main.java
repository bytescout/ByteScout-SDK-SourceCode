...
public class Main
{
        ...
        if (response.code() == 200)
        {
            ...
            if (!error)
            {
                ...
                if (uploadFile(webClient, uploadUrl, SourceFile))
                {
                    ...
                    MakePdfSearchable(webClient, DestinationFile, Password, Pages, Language, uploadedFileUrl);
                }
            }
            else
            {
    ...
    public static void MakePdfSearchable(OkHttpClient webClient, Path destinationFile,
        String password, String pages, String language, String uploadedFileUrl) throws IOException
    {
        ...
        String query = String.format(
                "https://localhost/pdf/makesearchable?name=%s&password=%s&pages=%s&lang=%s&url=%s",
