...
public class Main
{
        ...
        if (response.code() == 200)
        {
            ...
            if (!error)
            {
                ...
                if (uploadFile(webClient, uploadUrl, SourceFile))
                {
                    ...
                    PdfToJpeg(webClient, Password, Pages, uploadedFileUrl);
                }
            }
            else
            {
    ...
    public static void PdfToJpeg(OkHttpClient webClient, String password, String pages, String uploadedFileUrl) throws IOException
    {
        // Prepare URL for `PDF To JPEG` API call
        String query = String.format(
                "https://localhost/pdf/convert/to/jpg?password=%s&pages=%s&url=%s",
                password,
                pages,
                uploadedFileUrl);
