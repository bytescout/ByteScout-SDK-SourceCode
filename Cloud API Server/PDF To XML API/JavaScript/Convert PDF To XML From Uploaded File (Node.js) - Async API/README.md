...
### Follow this simple tutorial to learn convert PDF to XML from uploaded file (node to have PDF to XML API in JavaScript
...
function convertPdfToXml(uploadedFileUrl, password, pages, destinationFile) {
    ...
    var queryPath = `/pdf/convert/to/xml?name=${path.basename(destinationFile)}&password=${password}&pages=${pages}&url=${uploadedFileUrl}&async=True`;
    let reqOptions = {
        host: "localhost",
        path: encodeURI(queryPath),
        method: "GET" 
