...
public class Main
{
        ...
        if (response.code() == 200)
        {
            ...
            if (!error)
            {
                ...
                if (uploadFile(webClient, uploadUrl, SourceFile))
                {
                    ...
                    PdfToXml(webClient, DestinationFile, Password, Pages, uploadedFileUrl);
                }
            }
            else
            {
    ...
    public static void PdfToXml(OkHttpClient webClient, Path destinationFile,
        String password, String pages, String uploadedFileUrl) throws IOException
    {
        // Prepare URL for `PDF To XML` API call
        String query = String.format(
                "https://localhost/pdf/convert/to/xml?name=%s&password=%s&pages=%s&url=%s",
