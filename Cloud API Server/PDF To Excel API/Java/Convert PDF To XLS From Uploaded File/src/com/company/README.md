...
public class Main
{
        ...
        if (response.code() == 200)
        {
            ...
            if (!error)
            {
                ...
                if (uploadFile(webClient, uploadUrl, SourceFile))
                {
                    ...
                    PdfToXls(webClient, DestinationFile, Password, Pages, uploadedFileUrl);
                }
            }
            else
            {
    ...
    public static void PdfToXls(OkHttpClient webClient, Path destinationFile,
        String password, String pages, String uploadedFileUrl) throws IOException
    {
        // Prepare URL for `PDF To XLS` API call
        String query = String.format(
                "https://localhost/pdf/convert/to/xls?name=%s&password=%s&pages=%s&url=%s",
