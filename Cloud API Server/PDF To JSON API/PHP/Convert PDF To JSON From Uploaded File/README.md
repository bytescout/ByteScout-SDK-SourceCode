## How to convert PDF to JSON from uploaded file for PDF to JSON API in PHP using ByteScout Cloud API Server

### Step-by-step tutorial:How to convert PDF to JSON from uploaded file to have PDF to JSON API in PHP

These source code samples are listed and grouped by their programming language and functions they use. PDF to JSON API in PHP can be applied with ByteScout Cloud API Server. ByteScout Cloud API Server is the ready to use Web API Server that can be deployed in less than 30 minutes into your own in-house server or into private cloud server. Can store data on in-house local server based storage or in Amazon AWS S3 bucket. Processing data solely on the server using buil-in ByteScout powered engine, no cloud services are used to process your data!.

This simple and easy to understand sample source code in PHP for ByteScout Cloud API Server contains different functions and options you should do calling the API to implement PDF to JSON API. Open your PHP project and simply copy & paste the code and then run your app! Use of ByteScout Cloud API Server in PHP is also described in the documentation given along with the product.
    ...
    if ($status_code == 200)
    {
        ...
        if (curl_errno($curl) == 0)
        {
            ...
            if ($status_code == 200)
            {
                ...
                ExtractJSON($uploadedFileUrl, $pages);
            }
            else
            {
...
function ExtractJSON($uploadedFileUrl, $pages) 
{
    // Create URL
